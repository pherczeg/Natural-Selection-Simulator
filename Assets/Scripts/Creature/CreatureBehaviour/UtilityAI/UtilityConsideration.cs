...
public class UtilityConsideration
{
    ...
    private Func<UtilityAIContext, float> evaluator;
    ...
    public UtilityConsideration(string name, Func<UtilityAIContext, float> evaluator, float weight = 1f)
    {
        this.name = name;
    ...
    public void SetEvaluator(Func<UtilityAIContext, float> evaluator)
    {
        this.evaluator = evaluator;
    }
    ...
    {
        float rawScore = evaluator != null ? evaluator(context) : fallbackScore;
    ...
    private static float ClampUtilityScore(float score)
    {
        ...
        return UtilityAIScoreRules.Clamp01(score);
    }
}
