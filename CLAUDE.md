...
`Statistics` (singleton) collects time-series + cumulative counters (spawns, deaths by cause, reproduction, predation). `RealtimeSimulationStatsUI` shows a live overlay (click-selection and per-creature energy bars were removed in the full-DOTS migration, Phase 1). `GameManager` runs a **batch of N timed simulations**, exporting one CSV per run to `Application.persistentDataPath` and reloading the scene between runs. With `useEcsStatistics` on, core species aggregates come from `ECSStatisticsMirror` (refreshed each `updateInterval`) while uncovered fields stay polled — the two can be up to one interval apart.
